...
func main() {
	...
	connectedSize := make([]int, len(circuits))
	index := 0
	for _, v := range circuits {
		connectedSize[index] = len(v)
		index++
	}
	slices.SortFunc(connectedSize, func(a, b int) int { return cmp.Compare(b, a) })
	const largestCount = 3
	largest := connectedSize[:largestCount]
	product := 1
